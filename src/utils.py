...
User input: {user_input}"""
    reject_reason_info = {
        ...
        "(Our input filtering uses OpenAI GPT-4o-mini, which may result in false positives. "
        "We apologize for any inconvenience.)",
        4: "Sorry, WikiAutoGen cannot follow arbitrary instruction. Please input a topic you want to learn about. "
        "(Our input filtering uses OpenAI GPT-4o-mini, which may result in false positives. "
        "We apologize for any inconvenience.)",
    }
...
def format_reference(index, url_info):
    return f"[{index}] {url_info['title']}. {url_info['url']}"
...
def read_ref_file(file_path):
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        for line in file:
            ...
            ref_num = parts[0].strip('[]')
            ref_url = parts[1].split(' ')[-1]
            ref_dict[ref_num] = ref_url
    return ref_dict
...
def find_subsection_ranges(text):
    subsection_ranges = []
    subsection_start_indices = []
    for i, line in enumerate(text.splitlines()):
        if line.startswith('##'):
            subsection_start_indices.append(i)

    for i in range(len(subsection_start_indices)):
        start_index = sum(len(line) + 1 for line in text.splitlines()[:subsection_start_indices[i]])
        if i < len(subsection_start_indices) - 1:
            end_index = sum(len(line) + 1 for line in text.splitlines()[:subsection_start_indices[i + 1]]) - 1
        else:
            end_index = len(text)
        subsection_ranges.append((start_index, end_index))

    return subsection_ranges


def find_references(json_data, ref_dict, original_text):
    subsection_ranges = find_subsection_ranges(original_text)
    results = []
    for item in json_data:
        position_info = item["Position"]
        target_sentence = position_info.split("Target sentence: ")[1].split(". After sentence:")[0]
        title = item["Title"]

        start_index = original_text.find(target_sentence)
        if start_index == -1:
            print(f"Could not find 'Target sentence' in original text: {target_sentence}")
            continue

        subsection_start = 0
        subsection_end = len(original_text)
        for start, end in subsection_ranges:
            if start <= start_index < end:
                subsection_start = start
                subsection_end = end
                break

        search_start = max(subsection_start, start_index - 500)
        search_end = min(subsection_end, start_index + len(target_sentence) + 500)
        search_range = original_text[search_start:search_end]

        ref_nums = []
        index = 0
        while True:
            start = search_range.find('[', index)
            if start == -1:
                break
            end = search_range.find(']', start)
            if end != -1:
                ref_num = search_range[start + 1:end]
                if ref_num.isdigit():
                    ref_nums.append(ref_num)
            index = end + 1

        urls = [ref_dict[num] for num in ref_nums if num in ref_dict]
...
def write_to_txt(results, output_file_path):
    with open(output_file_path, 'w', encoding='utf-8') as file:
        for result in results:
            ...
            file.write(f"target_sentence: {target_sentence}\n")
...
def process_files(input_json_file, image_suggestions_json, original_text_file, output_txt_file, references_txt_file):
    try:
        with open(input_json_file, 'r', encoding='utf-8') as f:
            data = json.load(f)
        url_to_unified_index = data['url_to_unified_index']
        url_to_info = data['url_to_info']
        sorted_urls = sorted(url_to_unified_index.items(), key=lambda item: item[1])
        references = []

        for url, index in sorted_urls:
            url = url.strip('<>').strip()
            url_info = url_to_info[url]
            reference = format_reference(index, url_info)
            references.append(reference)

        with open(references_txt_file, 'w', encoding='utf-8') as f:
            f.write('\n'.join(references))
        print(f"References written to {references_txt_file}.")

        json_data = read_json_file(image_suggestions_json)
        ref_dict = read_ref_file(references_txt_file)
        original_text = read_original_text(original_text_file)
        results = find_references(json_data, ref_dict, original_text)
        write_to_txt(results, output_txt_file)
        print(f"Results have been written to {output_txt_file}")

    except FileNotFoundError:
        print(f"One of the input files not found. Check paths.")
...
def merge_text_files(file1_path, file2_path, output_path):
    try:
        ...
        combined_content += '# Reference \n\n'
        combined_content += content2

        with open(output_path, 'w', encoding='utf - 8') as output_file:
            output_file.write(combined_content)

        print(f"Files merged successfully. The result is saved in {output_path}")
    except FileNotFoundError:
...
def generate_html_and_markdown(article_file_path, input_file_path, output_folder):
    ...
    image_info_list = [(title, url[0] if url else None) for title, url in zip(titles, url_lists)]
    new_article = article

    image_placeholder_pattern = re.compile(r'\[IMAGE: (.*?), URL: (.*?)\]')
    section_pattern = re.compile(r'^(#+) (.*)$', re.MULTILINE)

    title_query_dict = {title: query for title, query in zip(titles, queries)}
    html_content = '<html><body>'
    html_content = section_pattern.sub(lambda m: f'<h{len(m.group(1))}>{m.group(2)}</h{len(m.group(1))}>', new_article)
    ...
    def html_image_replace(m):
        ...
        image_url = m.group(2)
        image_description = title_query_dict.get(image_name, image_name)
        html = f'<div style="text-align: center;"><img src="{image_url}" alt="{image_description}"></div>'
        html += f'<p style="text-align: center;">Fig.{image_count}: {image_description}</p>'
        image_count += 1
        return html

    html_content = image_placeholder_pattern.sub(html_image_replace, html_content)

    reference_pattern = re.compile(r'(<h1>Reference </h1>)(.*)', re.DOTALL)
    match = reference_pattern.search(html_content)
    if match:
        ref_content = match.group(2)
        new_ref_content = re.sub(r'(\[\d+\])', r'<br>\1', ref_content)
        html_content = html_content.replace(ref_content, new_ref_content)

    html_content += '</body></html>'
    html_file_path = os.path.join(output_folder, 'final_article.html')
    with open(html_file_path, 'w', encoding='utf-8') as html_file:
        html_file.write(html_content)
    print(f"HTML has generated in: {html_file_path}")

    markdown_content = new_article
    image_count = 1

    def markdown_image_replace(m):
        nonlocal image_count
        image_name = m.group(1)
        image_url = m.group(2)
        image_description = title_query_dict.get(image_name, image_name)
        markdown = f'\n![{image_description}]({image_url})\n'
        markdown += f'Fig.{image_count}: {image_description}\n'
        image_count += 1
        return markdown

    markdown_content = image_placeholder_pattern.sub(markdown_image_replace, markdown_content)
    markdown_file_path = os.path.join(output_folder, 'final_article.md')
    with open(markdown_file_path, 'w', encoding='utf-8') as markdown_file:
        markdown_file.write(markdown_content)
    print(f"Markdown has generated in: {markdown_file_path}")
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        ...
        while i < len(lines):
            if lines[i].startswith('query:'):
                query = lines[i].split('query: ')[1].strip()
                queries.append(query)
            elif lines[i].startswith('target_sentence:'):
                target_sentence = lines[i].split('target_sentence: ')[1].strip()
                target_sentences.append(target_sentence)
            elif lines[i].startswith('title:'):
                title = lines[i].split('title: ')[1].strip()
                titles.append(title)
            elif lines[i].startswith('urls:'):
                urls_str = lines[i].split('urls: ')[1].strip()
                if urls_str == '[]':
                    urls = []
                else:
            ...
            i += 1
    assert len(queries) == len(target_sentences) == len(titles) == len(url_lists), "The number of data is inconsistent and one-to-one correspondence cannot be guaranteed"
    return queries, target_sentences, titles, url_lists
