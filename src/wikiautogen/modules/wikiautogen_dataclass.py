...
class WikiArticle(Article):
    ...
    def _merge_new_info_to_references(
        ...
        for idx, info in enumerate(new_info_list):
            ...
            if url not in self.reference["url_to_unified_index"]:
                ...
                self.reference["url_to_info"][url] = info
            else:
                existing_snippets = self.reference["url_to_info"][url].snippets
                existing_snippets.extend(info.snippets)
    ...
    def from_outline_file(cls, topic: str, file_path: str):
        """
        Create WikiArticle class instance from outline file.
        """
        outline_str = FileIOHelper.load_str(file_path)
        return WikiArticle.from_outline_str(topic=topic, outline_str=outline_str)

    @classmethod
    def from_outline_str(cls, topic: str, outline_str: str):
        """
        Create WikiArticle class instance from outline only string.
        """
        lines = []
        try:
