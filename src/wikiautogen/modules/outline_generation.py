...
class OutlineGenerationModule(OutlineGenerationModule):
    """
    ...
    def generate_outline(
        ...
        old_outline: Optional[WikiArticle] = None,
        callback_handler: BaseCallbackHandler = None,
        return_draft_outline=False,
    ) -> Union[WikiArticle, Tuple[WikiArticle, WikiArticle]]:
        """
        ...
        Args:
            ...
            proposal (str): Additional proposal for the article outline.
            information_table (InformationTableGen): The information table containing the collected information.
            old_outline (Optional[WikiArticle]): An optional previous version of the article outline that can
        ...
        Returns:
            Union[WikiArticle, Tuple[WikiArticle, WikiArticle]]: Depending on the value of `return_draft_outline`,
                this method returns either a single `WikiArticle` object containing the final outline or a tuple of
                two  `WikiArticle` objects, the first containing the final outline and the second containing the
                draft outline.
        """
        ...
        article_with_outline_only = WikiArticle.from_outline_str(
            topic=topic, outline_str=result.outline
        )
        article_with_draft_outline_only = WikiArticle.from_outline_str(
        ...
        with dspy.settings.context(lm=self.engine):
            if old_outline is None:
                old_outline = ArticleTextProcessing.clean_up_outline(
                    self.draft_page_outline(topic=topic, proposal=proposal).outline
                )
                if callback_handler:
...
class WritePageOutline(dspy.Signature):
    ...
    proposal = dspy.InputField(prefix="Proposal for the outline: ", format=str)
...
class NaiveOutlineGen(dspy.Module):
    ...
    def forward(self, topic: str, proposal: str):
        outline = self.write_outline(topic=topic, proposal=proposal).outline
